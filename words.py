...
have	has, have	had	had
 ...
 take	take, takes	ok	taken
 look	look, looks	looked	looked
 thank	thank, thanks	thanked	thanked
 tell	tell, tells	ld	ld
 ...
 be, have, do, say, make, go, take, come, see, and get. I me we you mean means
 1. to be
2. to have
3. to ask
4. to do
...
'West', 'Bengal', 'Temple',
      'Top', 'Temples', 'To', 'Visit', 'In', 'Kolkata', 'place', 'location', 'is', 'was',
      'Here', 'are', 'the', 'famous', 'temples', 'at', 'where', 'when', 'what', 'how', 'whom',
      'Mandir','A', 'spellbinding', 'structure', 'built', 'in', 'the', 'pious', 'white', 'colour',
      'have', 'been', 'know', 'done', 'do', 'had', 'many',
      'have', 'has', 'had', 'do', 'does', 'did', 'done', 'know', 'knows', 'knew', 'known', 'think', 'thinks', 'thought', 'go', 'goes', 'went', 'gone', 'get', 'gets', 'got', 'gotten', 'say', 'says', 'said', 'want', 'wants', 'wanted', 'see', 'sees', 'saw', 'seen', 'mean', 'means', 'meant', 'let', 'lets', 'make', 'makes', 'made', 'come', 'comes', 'came', 'take', 'takes', 'ok', 'taken', 'look', 'looks', 'looked', 'thank', 'thanks', 'thanked', 'tell', 'tells', 'ld', 'put', 'puts', 'like', 'likes', 'liked', 'talk', 'talks', 'talked', 'need', 'needs', 'needed', 'believe', 'believes', 'believed', 'give', 'gives', 'gave', 'given', 'try', 'tries', 'tried', 'call', 'calls', 'called', 'find', 'finds', 'found', 'feel', 'feels', 'felt', 'happen', 'happens', 'happened', 'ask', 'asks', 'asked',
have have have have I I I I I I I I I I I I I I I I 
U U U U U U U U U 
V V V V V V
math
jain
...
wordfilterword = [re.sub(r"[^a-zA-Z]+", ' ', k) for k in words.split("\n")]
wordfilterword = re.sub('\,','',str(wordfilterword))
wordfilterword = re.sub(r'[^a-zA-Z]',' ',wordfilterword)
wordfilterword = re.findall(r'\S+', str(wordfilterword))

listingwords =  []
singlew = []
a = ['I', 'A']
for w in (wordfilterword):
    
    if len(w) <= 1:
        if w not in singlew:
            singlew.append(w)
    else:
        if w not in listingwords:
            listingwords.append(w)

def common_elemets(a, b):
    ce = [i for i in a if i in b]
    return ce

nc = common_elemets(a,singlew)
...
def join_elemets(a, b):
    ce = [i for i in a if i not in b]
    return ce

#sw = join_elemets(nc,listingwords)
